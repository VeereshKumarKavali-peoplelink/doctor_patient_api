...
const signUpAPI = async (request, response) => {
    ...
    try {
        const existingUser = await dbQueries.findOne(process.env.userTable, { email: email });
        if (existingUser) {
...
const loginAPI = async (request, response) => {
    ...
    try {
        const user = await dbQueries.findOne(process.env.userTable, { email: email });
        if (user) {
            const isPasswordMatched  = await bcrypt.compare(password, user.password);
            if (isPasswordMatched ) {
                console.log("inside Login API++", user._id)
                const jwtToken = jwt.sign({id: user._id, email: user.email, role: user.role }, process.env.JWT_SECRET, { expiresIn: '1h' });
